...
void Echange::setup()
{
    ...
    this->gui.add(this->tailleParticule.setup( "tailleParticule", 0.06, 0.01, 0.4 ) );
...
void Echange::updateOsc(Camera *camera, GpuProcess *process, Forme *forme, Background *background)
{
    ...
    while(this->receiver.hasWaitingMessages())
    {
        ...
        else if(argumentAdresse == "/max2P5/flockForces")
        {
            ...
            process->setDistanceFlocking(m.getArgAsFloat(0), m.getArgAsFloat(1), m.getArgAsFloat(2));
        } //(float entre 0 et 1)
